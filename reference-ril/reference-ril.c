...
static void requestGetHardwareConfig(void *data, size_t datalen, RIL_Token t)
{
   ...
   RIL_HardwareConfig hwCfg[2];

   RIL_UNUSED_PARM(data);
   RIL_UNUSED_PARM(datalen);

   hwCfg[0].type = RIL_HARDWARE_CONFIG_MODEM;
   sprintf(hwCfg[0].cfg.modem.uuid, "MODEM-CANNED");
   hwCfg[0].cfg.modem.state = RIL_HARDWARE_CONFIG_STATE_ENABLED;
   hwCfg[0].cfg.modem.rat = 0xFFFF; /* all RAT. */
   hwCfg[0].cfg.modem.maxVoice = 1;
   hwCfg[0].cfg.modem.maxData = 1;
   hwCfg[0].cfg.modem.maxStandby = 1;

   hwCfg[1].type = RIL_HARDWARE_CONFIG_SIM;
   sprintf(hwCfg[1].cfg.sim.uuid, "SIM-CANNED");
   sprintf(hwCfg[1].cfg.sim.modemUuid, "MODEM-CANNED");
   hwCfg[1].cfg.sim.state = RIL_HARDWARE_CONFIG_STATE_ENABLED;
