...
static RIL_RadioTechnology modem2tech[] = {
    RADIO_TECH_GPRS,
    RADIO_TECH_UMTS,
    RADIO_TECH_1xRTT,
    RADIO_TECH_EVDO_A,
...
static void requestSignalStrength(void *data, size_t datalen, RIL_Token t)
{
    ...
    response.GW_SignalStrength.signalStrength = ss;
    ...
    response.EVDO_SignalStrength.ecio = 90;
                                        ...
                                        size_t datalen, RIL_Token t)
{
    ...
    for (cur = p_response->p_intermediates, i = 0; cur; cur = cur->p_next, i++) {
        ...
        if (query_ctec(sMdmInfo, &current, NULL) < 0) {
            ...
            current = modem2tech[current];
        }
        asprintf(&responseStr[3], "%d", current);
