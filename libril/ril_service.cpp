...
Return<void> RadioImpl::iccOpenLogicalChannel(int32_t serial, const hidl_string& aid, uint8_t p2) {
    ...
    if (s_vendorFunctions->version < 15) {
        ...
        if (!copyHidlStringToRil(&openChannel.aidPtr, (const char *)aid, pRI)) {
            return Void();
        }

        s_vendorFunctions->onRequest(RIL_REQUEST_SIM_OPEN_CHANNEL, &openChannel, sizeof(openChannel), pRI);
