...
void radio::iccCardStatusResponse(int32_t slotId, int32_t responseType, int32_t serial, RIL_Errno e,
    ...
    if (radioService[slotId]->radioResponse != NULL) {
        ...
        RLOGE("responseGetSimStatus: radioService[%d]->radioResponse == NULL", slotId);
    }
}

void radio::radioStateChanged(int slotId, RIL_RadioState radioState) {
    RLOGD("radioStateChanged: radioState %d", radioState);
    if (radioService[slotId]->radioIndication != NULL) {
        radioService[slotId]->radioIndication->radioStateChanged((RadioState) radioState);
    } else {
...
void radio::registerService(RIL_RadioFunctions *callbacks, CommandInfo *commands) {
    ...
    for (int i = 0; i < simCount; i++) {
        ...
        std::string serviceName ("ril_service");
        serviceName += std::to_string(i);
        RLOGD("registerService: starting IRadio %s", serviceName.c_str());
        android::status_t status = radioService[i]->registerAsService(serviceName.c_str());
    }
