 ...
 * @param numfields number of string style member in the data profile structure
 * @param ... the variadic parameters are pointers to each string style member
 **/
template <typename T>
...
#ifdef MEMSET_FREED
            memsetString(dataProfiles[j].*ptr);
#endif
            free(dataProfiles[j].*ptr);
        }
    }
