...
bool dispatchScanRequest(int serial, int slotId, int request, const NetworkScanRequest& scan) {
    ...
    if (scan.specifiers.size() > MAX_RADIO_ACCESS_NETWORKS) {
        return false;
    }
    ...
    for (size_t i = 0; i < scan.specifiers.size(); ++i) {
        if (scan.specifiers[i].geranBands.size() > MAX_BANDS ||
            scan.specifiers[i].utranBands.size() > MAX_BANDS ||
            scan.specifiers[i].eutranBands.size() > MAX_BANDS ||
            scan.specifiers[i].channels.size() > MAX_CHANNELS) {
            return false;
        }
        ...
        switch (scan.specifiers[i].radioAccessNetwork) {
            case ::android::hardware::radio::V1_1::RadioAccessNetworks::GERAN:
                for (size_t k = 0; k < scan.specifiers[i].geranBands.size(); ++k) {
                    scan_request.specifiers[i].bands.geran_bands[k] =
                            (RIL_GeranBands) scan.specifiers[i].geranBands[k];
                }
                break;
            case ::android::hardware::radio::V1_1::RadioAccessNetworks::UTRAN:
                for (size_t k = 0; k < scan.specifiers[i].utranBands.size(); ++k) {
                    scan_request.specifiers[i].bands.utran_bands[k] =
                            (RIL_UtranBands) scan.specifiers[i].utranBands[k];
                }
                break;
            case ::android::hardware::radio::V1_1::RadioAccessNetworks::EUTRAN:
                for (size_t k = 0; k < scan.specifiers[i].eutranBands.size(); ++k) {
                    scan_request.specifiers[i].bands.eutran_bands[k] =
                            (RIL_EutranBands) scan.specifiers[i].eutranBands[k];
                }
                break;
            default:
                return false;
...
int radio::networkScanResultInd(int slotId,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
        ...
        result.status = (::android::hardware::radio::V1_1::ScanStatus) networkScanResult->status;
