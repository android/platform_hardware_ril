...
bool dispatchAdnRecord(int serial, int slotId, int request,
    ...
    adnInfo.email_elements = MIN(adnRecordInfo.emails.size(), RIL_MAX_NUM_EMAIL_COUNT);
    adnInfo.anr_elements = MIN(adnRecordInfo.adNumbers.size(), RIL_MAX_NUM_AD_COUNT);

    if (!copyHidlStringToRil(&adnInfo.name, adnRecordInfo.name, pRI, true)) {
        return false;
    }
...
Return<void> RadioImpl::updateAdnRecord(int32_t serial, const V1_2::AdnRecordInfo& adnRecordInfo) {
    ...
    RLOGD("getAdnRecord: serial %d", serial);
#endif
    dispatchAdnRecord(serial, mSlotId, RIL_REQUEST_UPDATE_ADN_RECORD, adnRecordInfo);
    return Void();
}
...
int radio::updateAdnRecordResponse(int slotId, int responseType, int serial, RIL_Errno e,
    ...
    if (radioService[slotId]->mRadioResponseV1_2 != NULL) {
        ...
        int ret = responseInt(responseInfo, serial, responseType, e, response, responseLen);
        Return<void> retStatus
                = radioService[slotId]->mRadioResponseV1_2->updateAdnRecordResponse(responseInfo,
                ret);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
...
int radio::adnRecordsReceivedInd(int slotId,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndicationV1_2 != NULL) {
        ...
        int numRecords = MIN(records->record_elements, RIL_NUM_ADN_RECORDS);
        ...
        for (int i = 0; i < numRecords; i++) {
            ...
            int emailElements = MIN(records->adn_record_info[i].email_elements,
                                    RIL_MAX_NUM_EMAIL_COUNT);
            ...
            int anrElements = MIN(records->adn_record_info[i].anr_elements,
                                RIL_MAX_NUM_AD_COUNT);
