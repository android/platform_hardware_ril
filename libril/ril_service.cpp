...
bool dispatchString(int serial, int slotId, int request, const char *str) {
    ...
    s_vendorFunctions->onRequest(request, (void *) str, sizeof(const char *), pRI);
...
bool dispatchStrings(int serial, int slotId, int request, int countStrings, ...) {
    ...
    const char **pStrings;
    pStrings = (const char **) calloc(countStrings, sizeof(const char *));
    if (pStrings == NULL) {
    ...
    for (int i = 0; i < countStrings; i++) {
        pStrings[i] = va_arg(ap, const char *);
    }
    va_end(ap);

    s_vendorFunctions->onRequest(request, pStrings, countStrings * sizeof(const char *), pRI);

    if (pStrings != NULL) {
#ifdef MEMSET_FREED
        memset(pStrings, 0, countStrings * sizeof(const char *));
#endif
        free(pStrings);
    }
    return true;
...
bool dispatchImsGsmSms(const ImsSmsMessage& message, RequestInfo *pRI) {
    ...
    pStrings[0] = (char *) message.gsmMessage[0].smscPdu.c_str();
    pStrings[1] = (char *) message.gsmMessage[0].pdu.c_str();
