...
void RilSapSocket::initSapSocket(const char *socketName,
    ...
    if (strcmp(socketName, HIDL1_SERVICE_NAME) == 0) {
...
#if (SIM_COUNT >= 2)
    if (strcmp(socketName, HIDL2_SERVICE_NAME) == 0) {
        if(!SocketExists(socketName)) {
...
#if (SIM_COUNT >= 3)
    if (strcmp(socketName, HIDL3_SERVICE_NAME) == 0) {
        if(!SocketExists(socketName)) {
...
#if (SIM_COUNT >= 4)
    if (strcmp(socketName, HIDL4_SERVICE_NAME) == 0) {
        if(!SocketExists(socketName)) {
