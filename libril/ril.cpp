...
responseBroadcastSms(Parcel &p, void *response, size_t responselen) {
    ...
    int pdu_len = responselen;
    if (s_callbacks.version >= 8) {
        puint8 = (uint8_t *) p_cur->pdu;
        pdu_len = p_cur->pdu_len;
    }
    if (1) { // print in decimal
        appendPrintBuf("%s\npdu_len=%d\n Pdu DEC:", printBuf, pdu_len);
        for (int i = 0, k=1; i < pdu_len; i += 8, k++) {
            appendPrintBuf("%s\n%d:  ", printBuf, k);
            for (int j = i; j < i + 8 && j < pdu_len; j++) {
                appendPrintBuf("%s%d, ", printBuf, puint8[j]);
            }
        }
    }
    if (1) { // print in Hex
        appendPrintBuf("%s\npdu_len=%d\n Pdu HEX:", printBuf, pdu_len);
        for (int i = 0, k=1; i < pdu_len; i += 8, k++) {
            appendPrintBuf("%s\n%d:  ", printBuf, k);
            for (int j = i; j < i + 8 && j < pdu_len; j++) {
