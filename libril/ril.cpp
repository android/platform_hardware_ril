...
typedef struct {
    int requestNumber;
    int (*responseFunction) (Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
...
static int responseInts(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseFailCause(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseStrings(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseString(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseVoid(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseCallList(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseSMS(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseSIM_IO(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseCallForwards(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseDataCallList(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseSetupDataCall(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseRaw(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseSsn(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseSimStatus(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseGsmBrSmsCnf(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseCdmaBrSmsCnf(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseCdmaSms(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseCellList(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseCdmaInformationRecords(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e,void *response, size_t responselen);
static int responseRilSignalStrength(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e,void *response, size_t responselen);
static int responseCallRing(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseCdmaSignalInfoRecord(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e,void *response, size_t responselen);
static int responseCdmaCallWaiting(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e,void *response, size_t responselen);
static int responseSimRefresh(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseCellInfoList(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseHardwareConfig(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseDcRtInfo(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseRadioCapability(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
static int responseSSData(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseLceStatus(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseLceData(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen);
static int responseActivityData(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen);
...
static int
responseInts(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e, void *response,
        size_t responselen) {
    int numInts;
...
static int responseStringsWithVersion(int version, Parcel &p, int requestNumber, int responseType,
        int token, RIL_Errno e, void *response, size_t responselen) {
    p.writeInt32(version);
    return responseStrings(p, requestNumber, responseType, token, e, response, responselen);
}
...
static int responseDataCallListV4(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen)
{
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
...
static int responseDataCallListV6(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen)
{
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
...
static int responseDataCallListV9(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen)
{
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
...
static int responseDataCallListV11(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen) {
    if (response == NULL && responselen != 0) {
                RLOGE("invalid response: NULL");
                return RIL_ERRNO_INVALID_RESPONSE;
        ...
        RIL_Errno e, void *response, size_t responselen)
{
    if (s_callbacks.version <= LAST_IMPRECISE_RIL_VERSION) {
        if (s_callbacks.version < 5) {
            ...
            return responseDataCallListV4(p, requestNumber, responseType, token, e, response,
                    responselen);
        } else if (responselen % sizeof(RIL_Data_Call_Response_v6) == 0) {
            return responseDataCallListV6(p, requestNumber, responseType, token, e, response,
                    responselen);
        } else if (responselen % sizeof(RIL_Data_Call_Response_v9) == 0) {
            return responseDataCallListV9(p, requestNumber, responseType, token, e, response,
                    responselen);
        } else {
            return responseDataCallListV11(p, requestNumber, responseType, token, e, response,
                    responselen);
        }
    } else { // RIL version >= 13
        if (responselen % sizeof(RIL_Data_Call_Response_v11) != 0) {
...
{
    if (s_callbacks.version < 5) {
        return responseStringsWithVersion(s_callbacks.version, p, requestNumber, responseType,
                token, e, response, responselen);
    } else {
        return responseDataCallList(p, requestNumber, responseType, token, e, response,
                responselen);
    }
}
...
static void responseRilSignalStrengthV5(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, RIL_SignalStrength_v10 *p_cur) {
...
static void responseRilSignalStrengthV6Extra(Parcel &p, int requestNumber, int responseType,
...
static void responseRilSignalStrengthV10(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, RIL_SignalStrength_v10 *p_cur) {
    responseRilSignalStrengthV5(p, requestNumber, responseType, token, e, p_cur);
    responseRilSignalStrengthV6Extra(p, requestNumber, responseType, token, e, p_cur);
...
static int responseRilSignalStrength(Parcel &p, int requestNumber, int responseType, int token,
    ...
    if (s_callbacks.version <= LAST_IMPRECISE_RIL_VERSION) {
        ...
        p_cur = ((RIL_SignalStrength_v10 *) response);
        responseRilSignalStrengthV10(p, requestNumber, responseType, token, e, p_cur);
    }
    startResponse;
...
static int responseCallRing(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen) {
    if ((response == NULL) || (responselen == 0)) {
        return responseVoid(p, requestNumber, responseType, token, e, response, responselen);
    } else {
        return responseCdmaSignalInfoRecord(p, requestNumber, responseType, token, e, response,
                responselen);
    }
}
...
static void responseSimRefreshV7(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response) {
...
static int responseSimRefresh(Parcel &p, int requestNumber, int responseType, int token,
    ...
    if (s_callbacks.version <= LAST_IMPRECISE_RIL_VERSION) {
        if (s_callbacks.version >= 7) {
            responseSimRefreshV7(p, requestNumber, responseType, token, e, response);
        } else {
...
static int responseCellInfoListV6(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen) {
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
...
static int responseCellInfoListV12(Parcel &p, int requestNumber, int responseType, int token,
        RIL_Errno e, void *response, size_t responselen) {
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
        ...
        RIL_Errno e, void *response, size_t responselen)
{
    if (s_callbacks.version <= LAST_IMPRECISE_RIL_VERSION) {
        if (s_callbacks.version < 12) {
            ...
            return responseCellInfoListV6(p, requestNumber, responseType, token, e, response,
                    responselen);
        } else {
            RLOGD("responseCellInfoList: v12");
            return responseCellInfoListV12(p, requestNumber, responseType, token, e, response,
                    responselen);
        }
    } else { // RIL version >= 13
        if (responselen % sizeof(RIL_CellInfo_v12) != 0) {
...
static int responseSimStatus(Parcel &p, int requestNumber, int responseType, int token, RIL_Errno e,
        void *response, size_t responselen) {
    RLOGE("responseSimStatus: token %d", token);
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
    }

    radio::iccCardStatusResponse(responseType, token, e, response, responselen);
...
RIL_onRequestComplete(RIL_Token t, RIL_Errno e, void *response, size_t responselen) {
    ...
    if (pRI->cancelled == 0) {
        ...
        if (response != NULL) {
            ...
            ret = pRI->pCI->responseFunction(p, pRI->pCI->requestNumber, responseType, pRI->token,
...
#endif
{
    ...
    ret = s_unsolResponses[unsolResponseIndex].responseFunction(
            p, unsolResponse, responseType, 0, RIL_E_SUCCESS, const_cast<void*>(data), datalen);
    if (ret != 0) {
    ...
    if (unsolResponse != RIL_UNSOL_RESPONSE_RADIO_STATE_CHANGED) {
        ...
        radio::radioStateChanged(newState);
    }
