...
static int responseGetDcRtInfo(Parcel &p, void *response, size_t responselen);
static int responseDcRtInfoChanged(Parcel &p, void *response, size_t responselen);
...
static void marshallDcRtInfo(Parcel &p, RIL_DcRtInfo *pDcRtInfo, int num)
{
    for (int i = 0; i < num; i++) {
...
static int responseGetDcRtInfo(Parcel &p, void *response, size_t responselen)
{
    ...
    startResponse;
    marshallDcRtInfo(p, (RIL_DcRtInfo *)response, 1);
    removeLastChar;
    closeResponse;
...
static int responseDcRtInfoChanged(Parcel &p, void *response, size_t responselen)
{
    if (responselen % sizeof(RIL_DcRtInfo) != 0) {
        RLOGE("invalid response length %d expected multiple of %d",
                (int)responselen, (int)sizeof(RIL_DcRtInfo));
        return RIL_ERRNO_INVALID_RESPONSE;
    }
    ...
    startResponse;
    marshallDcRtInfo(p, (RIL_DcRtInfo *)response, num);
    removeLastChar;
    closeResponse;
