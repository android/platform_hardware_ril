...
extern "C"
char radio_service_name_base[MAX_SOCKET_NAME_LENGTH] = HIDL_SERVICE_NAME_BASE;
extern "C"
char radio_service_name[MAX_SOCKET_NAME_LENGTH] = HIDL1_SERVICE_NAME;
/*******************************************************************/
...
char * RIL_getRilSocketName() {
    return radio_service_name;
}

extern "C"
void RIL_setRilSocketName(const char * s) {
    strncpy(radio_service_name, s, MAX_SOCKET_NAME_LENGTH);
}
...
RIL_register_socket (RIL_RadioFunctions *(*Init)(const struct RIL_Env *, int, char **),
    ...
    if(Init) {
        ...
        switch(socketType) {
            case RIL_SAP_SOCKET:
                RilSapSocket::initSapSocket(HIDL1_SERVICE_NAME, UimFuncs);

#if (SIM_COUNT >= 2)
                RilSapSocket::initSapSocket(HIDL2_SERVICE_NAME, UimFuncs);
#endif

#if (SIM_COUNT >= 3)
                RilSapSocket::initSapSocket(HIDL3_SERVICE_NAME, UimFuncs);
#endif

#if (SIM_COUNT >= 4)
                RilSapSocket::initSapSocket(HIDL4_SERVICE_NAME, UimFuncs);
#endif
                break;
            default:;
        }
