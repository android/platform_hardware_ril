...
static int responseCellInfo(Parcel &p, void *response, size_t responselen)
{
    ...
    int i;
    for (i = 0; i < num; i++) {
        appendPrintBuf("%s[%d: type=%d,registered=%d,time=%ld", printBuf, i,
            p_cur->cellInfoType, p_cur->registered, p_cur->time);
        p.writeInt32((int)p_cur->cellInfoType);
        p.writeInt32(p_cur->registered);
        p.writeInt64(p_cur->time);
        switch(p_cur->cellInfoType) {
            case RIL_CELL_INFO_TYPE_GSM: {
                appendPrintBuf("%s GSM id: mcc=%d,mnc=%d,lac=%d,cid=%d,psc=%d SS: gsmSS ss=%d,ber=%d],", printBuf,
            ...
            case RIL_CELL_INFO_TYPE_CDMA: {
                ...
                appendPrintBuf("%s evdoSS: dbm=%d,ecio=%d,snr=%d cdmaSS: dbm=%d ecio=%d", printBuf,
