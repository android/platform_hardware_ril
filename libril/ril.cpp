   ...
   for (i = 0; i < num; i++) {
      switch (p_cur[i].type) {
         case RIL_HARDWARE_CONFIG_MODEM: {
            writeStringToParcel(p, p_cur[i].cfg.modem.uuid);
            p.writeInt32((int)p_cur[i].cfg.modem.state);
            ...
            appendPrintBuf("%s modem: uuid=%s,state=%d,rat=%08x,maxV=%d,maxD=%d,maxS=%d", printBuf,
               p_cur[i].cfg.modem.uuid, (int)p_cur[i].cfg.modem.state, p_cur[i].cfg.modem.rat,
               p_cur[i].cfg.modem.maxVoice, p_cur[i].cfg.modem.maxData, p_cur[i].cfg.modem.maxStandby);
            break;
         }
         case RIL_HARDWARE_CONFIG_SIM: {
            writeStringToParcel(p, p_cur[i].cfg.sim.uuid);
            writeStringToParcel(p, p_cur[i].cfg.sim.modemUuid);
            p.writeInt32((int)p_cur[i].cfg.sim.state);

            appendPrintBuf("%s sim: uuid=%s,modem-uuid=%s,state=%d", printBuf,
               p_cur[i].cfg.sim.uuid, (int)p_cur[i].cfg.sim.state, p_cur[i].cfg.sim.modemUuid);
            break;
         }
      }
   }
