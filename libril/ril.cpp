...
static int responseCellInfoList(Parcel &p, void *response, size_t responselen)
{
    RLOGE("responseCellInfoList: E response=%p responselen=%d", response, responselen);
    if (response == NULL && responselen != 0) {
        RLOGE("invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
    }
    ...
    p.writeInt32(num);
    RLOGE("responseCellInfoList: num=%d, size cellinfo=%d", num, sizeof(RIL_CellInfo));

    RIL_CellInfo *p_cur = (RIL_CellInfo *) response;
    startResponse;
    int i;
    for (i = 0; i < num; i++) {
        appendPrintBuf("%s[%d: type=%d,registered=%d,timeStampType=%d,timeStamp=%ld", printBuf, i,
