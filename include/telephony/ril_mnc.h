...
static inline std::string decode(int mnc) {
    if (mnc == INT_MAX) return "";

    char mncNumDigits = (mnc >> (sizeof(int) * 8 - 4)) & 0xF;

    mnc = (mnc << 4) >> 4;
    if (mnc > 999 || mnc < 0) return "";
    ...
    switch (mncNumDigits) {
        case 0:
            ...
            return std::to_string(mnc);

        case 2:
        case 3: {
            char mncStr[4] = {0};
            snprintf(mncStr, sizeof(mncStr), "%03.3d", mnc);
            return mncStr + 3 - mncNumDigits;
        }
