 ...
 *                    New commands added: RIL_REQUEST_DEVICE_STATE,
...
typedef struct {
    int isInteractive;              // True if the device is in interactive state.
    int isDeviceIdleMode;           // True if the device is in idle state.
    int isPowerSaveMode;            // True if the device is in power save mode.
    int isCharging;                 // True if the device is in charging.
    int isHighDataTrafficExpected;  // True if high data traffic is expected, for example, when
                                    // tethering is on.
} RIL_DeviceState;

typedef enum {
    RIL_UR_SIGNAL_STRENGTH          = 0x01; // When this bit is set, modem should always send the
                                            // signal strength update, otherwise should suppress it.
    RIL_UR_FULL_VOICE_REGISTRATION  = 0x02; // When this bit is set, modem should always send the
                                            // full voice registration state. When this bit is not
                                            // set, modem should suppress the unsolicited response
                                            // only if insignificant fields change (e.g. cell info).
                                            // Modem should continue send the voice registration
                                            // state update when significant fields are updated
                                            // (e.g. service state) even when this bit is not set.
    RIL_UR_FULL_DATA_REGISTRATION   = 0x04; // When this bit is set, modem should always send the
                                            // full data registration state. When this bit is not
                                            // set, modem should suppress the unsolicited response
                                            // only if insignificant fields change (e.g. cell info).
                                            // Modem should continue send the data registration
                                            // state update when significant fields are updated
                                            // (e.g. service state) even when this bit is not set.
    RIL_UR_DATA_CALL_LIST_CHANGED   = 0x08; // When this bit is set, modem should send the data call
                                            // list changed unsolicited response
                                            // RIL_UNSOL_DATA_CALL_LIST_CHANGED. Otherwise modem
                                            // should suppress the unsolicited response.
 ...
 *                          For example, "IP", "IPV6", "IPV4V6", or "PPP".
 * ((const char **)data)[7] is the bitmask of APN type in decimal string format. The
 *                          bitmask will encapsulate the following values:
 *                          ia,mms,agps,supl,hipri,fota,dun,ims,default.
 * ((const char **)data)[8] is the roaming connection type to request. Must be one of the
 *                          PDP_type values in TS 27.007 section 10.1.1.
 *                          For example, "IP", "IPV6", "IPV4V6", or "PPP".
 ...
 * ((const char **)data)[13] is the MVNO match data.Use the following examples.
 *                           SPN: A MOBILE, BEN NL, ...
 *                           IMSI: 302720x94, 2060188, ...
 *                           GID: 4E, 33, ...
 ...
 *  RADIO_NOT_AVAILABLE (radio resetting)
 */
#define RIL_REQUEST_DEVICE_STATE 138
 ...
 * Modem can perform power saving based on the provided device state.
 * "data" is an const RIL_UnsolicitedResponseFilter **
 * "datalen" is count * sizeof(const RIL_UnsolicitedResponseFilter *)
 * "response" is NULL
 *
 * Valid errors:
 *  SUCCESS
