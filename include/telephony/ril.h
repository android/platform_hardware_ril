...
typedef enum {
    ...
    PDP_SUCCESS = 0x0,                             /* data call succeeded */
    ...
    PDP_FAIL_PREF_RADIO_TECH_CHANGED = -3, /* current/preferred technology has changed or change in progress,
                                              retry with parameters appropriate for new technology */
    PDP_FAIL_RADIO_POWER_OFF = -4,         /* data call was rejected because radio was resetting,
                                              or powering off */
    PDP_FAIL_TETHERED_CALL_ACTIVE = -5,    /* data call was rejected by the modem because tethered
                                              mode was up on same APN/data profile */
 ...
 * ((char **)response)[2] a space-separated list of IP addresses for this interface for GSM/UMTS
                          and NULL for CDMA
 * ((char **)response)[3] an integer, the reason why the call failed. If the call failed, must be
 *                        one of the values of RIL_DataCallFailCause. If the call succeeded, must be 0.
 *                        The call can also succeed with partial failure, as is the case with failure
 *                        causes 0x32, 0x33 and 0x34.
 *
